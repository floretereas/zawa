...
- `core/`: Pure game logic. Rules live here.
  - `domain/`: types
  - `engine/`: state transitions (`step`)
  - `ports/`: interfaces for future storage/transport
  - `local/`: local adapters used by V1
- `web/`: UI only. It renders state and sends events. No rules.
- `docs/`: Notes for myself: rules, state machine, and small design decisions.
